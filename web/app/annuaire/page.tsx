...
import { membres, tousLesTags } from "@/lib/membres";
...
export default function AnnuairePage() {
  ...
  const membresFiltres = useMemo(() => {
    ...
    return membres.filter((m) =>
      filtres.every((f) => m.langages.includes(f))
    );
  }, [filtres]);

  const totalPages = Math.ceil(membresFiltres.length / PAR_PAGE);
  const membresPage = membresFiltres.slice(
    (page - 1) * PAR_PAGE,
    page * PAR_PAGE
  );
      ...
      <section className="w-full max-w-7xl mx-auto px-6 pb-4">
        <div className="flex justify-center gap-16 border-2 border-[#2d3235] py-6 bg-white retro-shadow">
          ...
          <div className="text-center">
            ...
            <div className="text-xs font-semibold uppercase tracking-widest text-[#5a5f63] mt-1">Langages</div>
          </div>
        </div>
      </section>
          ...
          {tousLesTags.map((tag) => (
            <button
              ...
              className={`px-3 py-1.5 text-sm font-medium border-2 transition-colors ${
                filtres.includes(tag)
                  ? "bg-[#2d3235] text-[#efeadd] border-[#2d3235]"
                  : "bg-white text-[#2d3235] border-[#2d3235] hover:bg-[#efeadd]"
              }`}
            >
              {tag}
            </button>
            ...
            {membresPage.map((m, i) => (
              ...
              >
                ...
                <div className="p-4 flex flex-col flex-1 gap-3">
                  <div>
                    <h3 className="font-semibold text-[#2d3235] leading-snug">
                      {m.prenom} {m.nom}
                    </h3>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1 flex-1">
                    {m.langages.map((lang) => (
                      <span
                        key={lang}
                        className={`text-xs px-2 py-0.5 border font-medium ${getBadgeClass(lang)}`}
                      >
                        {lang}
                      </span>
                    ))}
                  ...
                  <div className="flex gap-2 pt-2 border-t border-gray-100">
                    {m.linkedin ? (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="p-2 border-2 border-[#2d3235] hover:bg-[#2d3235] hover:text-white transition-colors"
                        aria-label="LinkedIn"
                      >
                        <Linkedin className="w-4 h-4" />
                      </a>
                    ) : (
                      <span className="p-2 border-2 border-gray-200 text-gray-300 cursor-not-allowed">
                        <Linkedin className="w-4 h-4" />
                      </span>
                    )}
                    {m.facebook ? (
          ...
          <div className="flex justify-center items-center gap-2 mt-14">
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
              <button
                key={p}
                onClick={() => setPage(p)}
                className={`w-10 h-10 border-2 border-[#2d3235] font-semibold transition-colors ${
                  p === page
                    ? "bg-[#2d3235] text-[#efeadd]"
                    : "bg-white hover:bg-[#efeadd]"
                }`}
              >
                {p}
              </button>
